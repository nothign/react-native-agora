...
class RtcEngine {
  ...
  public static on(eventType: string, listener: (...args: any[]) => any): any {
    ...
    if (['remoteAudioStats', 'remoteVideoStats'].indexOf(eventType) != -1) {
      AgoraEventEmitter.addListener(`${RtcEngine.AG_PREFIX}${eventType}`, (args) => {
  ...
  public static setAudioSessionOperationRestriction() {
    if (Platform.OS != 'ios')
      throw Error(
        `setAudioSessionOperationRestriction is not support on your platform. Please check the details in react-native-agora docs`
      );
  ...
  static setVideoEncoderConfiguration(config: VideoEncoderConfig): Promise<any> {
    return Agora.setVideoEncoderConfiguration(config);
  }
